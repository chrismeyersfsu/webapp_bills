...
class CampusTerm(MethodView):
	url = "http://www.neebo.com/Course/florida-state-university"
	regexSelect = '<select id=\"Term_Id\" name=\"Term.Id\" style=\"margin: 0;\">(?P<terms_data>.*?)</select>'
	regexOption = '<option value=\"(.*?)\">(.*?)</option>'
	...
	def get(self):
		...
		try:
			...
			pattern = re.compile(self.regexSelect, re.MULTILINE | re.DOTALL)
			...
			while True:
				...
				if term_id is "":
					pos = match.end(0)
					continue;
				results.append({ 'campustermId' : 'NA|NA', 'campusId' : 'NA', 'termId' : term_id, 'campusName' : 'NA' })
				pos = match.end(0)

			return make_response(simplejson.dumps(results), error)
		except urllib2.URLError, e:
			handleError(e)


class Department(MethodView):
	url = "http://www.neebo.com/Course/GetDepartments?"
	regexDept = '<a href=\"javascript:void\(0\)\" class=\"dept\" id=\"(?P<dept_id>.*?)\">(.*?)<td style=\"width:80px; vertical-align:middle; font-weight:bold; text-align:right;\">\s+(?P<dept_abrev>.*?)<br/>(.*?)<td style=\"vertical-align:middle; padding-left:6px;\">\s+(?P<dept_name>.*?)\s+</td>'

	def get(self):
		#dept = request.args.get('campus')
		term = request.args.get('term')
		urlFull = self.url+'termId='+term

		try:
			result = urllib2.urlopen(urlFull)
			data = result.read()
			
			pattern = re.compile(self.regexDept, re.MULTILINE | re.DOTALL)
			pos = 0;
			results = list()
			while True:
				...
				if match is None:
					break;
				results.append({ 'deptId' : match.group('dept_id'), 'deptAbrev': match.group('dept_abrev'), 'deptName' : match.group('dept_name') })
				pos = match.end(0)
		
			return make_response(simplejson.dumps(results))
		except urllib2.URLError, e:
			handleError(e)
...
if __name__ == "__main__":
	app.add_url_rule("/campusterm/", view_func=CampusTerm.as_view('campusterm'))
	app.add_url_rule("/dept/", view_func=Department.as_view('dept'))
	app.run(debug=True)
